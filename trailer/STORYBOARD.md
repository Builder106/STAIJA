...
| 3 | 270–480 | The solution | `src/scenes/TheSolution.tsx` — phone device shot, 60-frame ±35° sway loop from `public/staija_phone_*.png` (source: `public/phone.blend`) |
...
- `public/phone.blend` + `public/staija_phone_0001–0060.png` — the device shot.
  No scripted regen path exists (unlike MicroMatch's `laptop.py`), so the PNG
  sequence stays tracked in git rather than gitignored.
...
- `staija_mobile.png` — a full-page mobile screenshot generated by
  `screenshot.js` (also moved here), not referenced by any scene.
